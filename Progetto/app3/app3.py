...
keycloak = oauth.register(
    name='keycloak',
    client_id='app3',
    client_secret='2DxT3WxCEl5u6CNBF0OhSMJazsgmRuHG',
...
# Carica il modello
model = SimpleNet()
model.load_state_dict(torch.load('ship_truck_classifier.pth', map_location=torch.device('cpu')))
model.eval()
...
@allowed_users_only(['user1', 'user2', 'user3'])  # Solo questi due possono accedere
def index():
    ...
    return render_template('shiptruck.html', prediction=prediction, filename=filename, user=session['user'])
