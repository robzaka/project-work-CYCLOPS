...
keycloak = oauth.register(
    name='keycloak',
    client_id='app1',
    client_secret='vkUEC1GLy7UZ0dh0vaQqqkkpQqqMls59',
...
# Carica il modello
model = SimpleNet()
model.load_state_dict(torch.load('plane_car_classifier.pth', map_location=torch.device('cpu')))
model.eval()
...
@allowed_users_only(['user1'])  # Solo questo
def index():
    ...
    return render_template('carplane.html', prediction=prediction, filename=filename, user=session['user'])

@app.errorhandler(403)
def forbidden(e):
    return render_template("carplane.html", error_code=403), 403
